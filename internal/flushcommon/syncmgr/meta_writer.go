...
type MetaWriter interface {
	...
	UpdateTextSourceSync(context.Context, *TextSourceSyncTask) error
...
func (b *brokerMetaWriter) UpdateTextSourceSync(ctx context.Context, task *TextSourceSyncTask) error {
	...
	log.Info("SaveBinlogPath for text source sync",
	...
	err := retry.Handle(ctx, func() (bool, error) {
		err := b.broker.SaveBinlogPaths(ctx, req)
		if errors.IsAny(err, merr.ErrSegmentNotFound, merr.ErrChannelNotFound) {
			log.Warn("meta error found, fail text source sync",
	...
	if err != nil {
		log.Warn("failed to SaveBinlogPaths for text source sync",
			zap.Int64("segmentID", task.segmentID),
			zap.Error(err))
		return err
	}
