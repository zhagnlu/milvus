...
type fakeCommitTextFlushSource struct {
	commits []int64
}

func (s *fakeCommitTextFlushSource) CurrentOffset() int64 {
	return 10
}

func (s *fakeCommitTextFlushSource) FlushTextData(context.Context, int64, int64, *TextFlushConfig) (*TextFlushResult, error) {
	return &TextFlushResult{ManifestPath: "manifest", NumRows: 10}, nil
}

func (s *fakeCommitTextFlushSource) Release() {
}

func (s *fakeCommitTextFlushSource) CommitTextFlush(targetOffset int64) {
	s.commits = append(s.commits, targetOffset)
}

func TestTextSourceSyncTaskCommitRetainedSourceOnlyOnFinalization(t *testing.T) {
	run := func(t *testing.T, finalize func(*TextSourceSyncTask), expectCommit bool) {
		segmentID := int64(1)
		...
		source := &fakeCommitTextFlushSource{}
	...
	t.Run("flush_finalization_commits_retained_source", func(t *testing.T) {
		run(t, func(task *TextSourceSyncTask) {
			task.WithFlush()
		}, true)
	})
