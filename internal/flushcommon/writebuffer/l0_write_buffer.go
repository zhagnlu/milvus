...
func (wb *l0WriteBuffer) BufferData(insertData []*InsertData, deleteMsgs []*msgstream.DeleteMsg, startPos, endPos *msgpb.MsgPosition, schemaVersion int32) error {
	...
	for _, inData := range insertData {
		if wb.hasTextFields {
			targetOffset := wb.textTargetOffset(inData.segmentID, inData.rowNum)
			decision := wb.decideTextFlushSource(inData.segmentID, targetOffset, endPos)
			if decision.sourceType == metacache.FlushSourceGrowing {
				wb.recordTextGrowingProgress(inData, startPos, endPos, schemaVersion, targetOffset)
				continue
			}
		}
