...
type fakeTextFlushSource struct {
	currentOffset int64
	flushFunc     func(context.Context, int64, int64, *syncmgr.TextFlushConfig) (*syncmgr.TextFlushResult, error)
	releaseFunc   func()
}

func (s fakeTextFlushSource) CurrentOffset() int64 {
...
func (s fakeTextFlushSource) FlushTextData(ctx context.Context, startOffset, endOffset int64, config *syncmgr.TextFlushConfig) (*syncmgr.TextFlushResult, error) {
	if s.flushFunc != nil {
		return s.flushFunc(ctx, startOffset, endOffset, config)
	}
	return &syncmgr.TextFlushResult{ManifestPath: "manifest", NumRows: 10}, nil
}

func (s fakeTextFlushSource) Release() {
	if s.releaseFunc != nil {
		s.releaseFunc()
	}
}

type fakeTextSourceProvider struct {
	source syncmgr.TextFlushSource
	state  syncmgr.TextSourceState
}

func (p fakeTextSourceProvider) GetTextFlushSource(int64, int64, *msgpb.MsgPosition) (syncmgr.TextFlushSource, syncmgr.TextSourceState) {
	return p.source, p.state
}

type fakeTextReleaseHandoffProvider struct {
	fakeTextSourceProvider
	fenceTs  uint64
	segments []syncmgr.TextReleaseHandoffSegment
}

func (p *fakeTextReleaseHandoffProvider) PrepareTextReleaseHandoff(ctx context.Context, fenceTs uint64, segments []syncmgr.TextReleaseHandoffSegment) error {
	p.fenceTs = fenceTs
	p.segments = append([]syncmgr.TextReleaseHandoffSegment(nil), segments...)
	return nil
}

func (p *fakeTextReleaseHandoffProvider) IsReleasePrepared(segmentID int64, checkpointTs uint64) bool {
...
func (p *fakeTextReleaseHandoffProvider) IsReleaseAllowed(segmentID int64, checkpointTs uint64) bool {
	if p.fenceTs != 0 && checkpointTs > p.fenceTs {
		return false
	}
	for _, segment := range p.segments {
...
func (p *fakeTextReleaseHandoffProvider) ClearReleasePrepared(segmentID int64) {
}

func (p *fakeTextReleaseHandoffProvider) ReleasePreparedSegments() []int64 {
	return nil
}
...
func (s *L0WriteBufferSuite) TestBufferDataTextSourceMode() {
	s.Run("no_usable_source_buffers_payload", func() {
		...
		// insert: (1) decideTextFlushSource reads the sticky flushSourceMode,
		// (2) textBaseOffset reads the row counter, (3) CreateNewGrowingSegment
	...
	s.Run("usable_source_records_progress_and_pins_checkpoint", func() {
		...
		wb, err := NewL0WriteBuffer(s.channelName, metacache, s.syncMgr, &writeBufferOption{
			idAllocator:             s.allocator,
			textSourceRetryInterval: time.Hour,
			textGrowingSourceResolver: func(segmentID int64, targetOffset int64, _ *msgpb.MsgPosition) (syncmgr.TextFlushSource, syncmgr.TextSourceState) {
				resolveCalls++
				resolvedSegmentID = segmentID
				resolvedTargetOffset = targetOffset
				if resolveCalls == 1 {
					return fakeTextFlushSource{}, syncmgr.TextSourceUsable
				}
		...
		// 3 GetSegmentByID calls per BufferData first-insert path: see comment
		// in no_usable_source_buffers_payload above.
		metacache.EXPECT().GetSegmentByID(int64(1001)).Return(nil, false).Times(3)
		metacache.EXPECT().AddSegment(mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return()
		metacache.EXPECT().UpdateSegments(mock.Anything, mock.Anything).Return()
		...
		s.Contains(l0wb.textGrowingProgress, int64(1001))
	...
	s.Run("writebuffer_source_is_sticky_after_payload_yielded", func() {
		...
		sourceState := syncmgr.TextSourceUnavailable
		wb, err := NewL0WriteBuffer(s.channelName, metacache, s.syncMgr, &writeBufferOption{
			idAllocator:             s.allocator,
			textSourceRetryInterval: time.Hour,
			textGrowingSourceResolver: func(segmentID int64, targetOffset int64, _ *msgpb.MsgPosition) (syncmgr.TextFlushSource, syncmgr.TextSourceState) {
				return fakeTextFlushSource{}, sourceState
			},
		})
		s.NoError(err)
		...
		s.NotContains(l0wb.textGrowingProgress, int64(1101))

		_, err = l0wb.getSyncTask(context.Background(), 1101)
		s.NoError(err)
		s.Nil(l0wb.buffers[int64(1101)])

		sourceState = syncmgr.TextSourceUsable
	...
	s.Run("close_stops_text_source_retry_timer", func() {
		textSchema := s.textSchema()
		metacache := s.newTextRealMetaCache(textSchema)
		sourceState := syncmgr.TextSourcePending
		wb, err := NewL0WriteBuffer(s.channelName, metacache, s.syncMgr, &writeBufferOption{
			idAllocator:             s.allocator,
			textSourceRetryInterval: 10 * time.Millisecond,
			textGrowingSourceResolver: func(segmentID int64, targetOffset int64, _ *msgpb.MsgPosition) (syncmgr.TextFlushSource, syncmgr.TextSourceState) {
				return fakeTextFlushSource{}, sourceState
			},
		})
		...
		l0wb := wb.(*l0WriteBuffer)
		s.True(l0wb.textSourceRetryScheduled)
		wb.Close(context.Background(), false)
		sourceState = syncmgr.TextSourceUsable
		time.Sleep(50 * time.Millisecond)
	})

	s.Run("drop_close_skips_unavailable_text_source", func() {
		textSchema := s.textSchema()
		metacache := s.newTextRealMetaCache(textSchema)
		...
		wb, err := NewL0WriteBuffer(s.channelName, metacache, s.syncMgr, &writeBufferOption{
			...
			textSourceRetryInterval: time.Hour,
			textGrowingSourceResolver: func(segmentID int64, targetOffset int64, _ *msgpb.MsgPosition) (syncmgr.TextFlushSource, syncmgr.TextSourceState) {
				return fakeTextFlushSource{}, syncmgr.TextSourcePending
			},
		})
		s.NoError(err)
		...
		l0wb := wb.(*l0WriteBuffer)
		s.NotContains(l0wb.textGrowingProgress, int64(1103))
	...
	s.Run("pending_source_records_progress_instead_of_falling_back_to_writebuffer", func() {
		...
		// 3 GetSegmentByID calls per BufferData first-insert path: see comment
		// in no_usable_source_buffers_payload above.
		metacache.EXPECT().GetSegmentByID(int64(1002)).Return(nil, false).Times(3)
		metacache.EXPECT().AddSegment(mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return()
		metacache.EXPECT().UpdateSegments(mock.Anything, mock.Anything).Return()
		...
		s.Contains(l0wb.textGrowingProgress, int64(1002))
		s.EqualValues(100, wb.GetCheckpoint().GetTimestamp())
	})
	...
	s.Run("pending_source_syncs_after_it_becomes_usable_and_acks_progress", func() {
		...
		s.Contains(l0wb.textGrowingProgress, int64(1003))
		s.EqualValues(100, wb.GetCheckpoint().GetTimestamp())

		sourceState = syncmgr.TextSourceUsable
		taskCh := make(chan *syncmgr.TextSourceSyncTask, 1)
		s.syncMgr.EXPECT().SyncData(mock.Anything, mock.AnythingOfType("*syncmgr.TextSourceSyncTask"), mock.Anything).
			RunAndReturn(func(ctx context.Context, task syncmgr.Task, callbacks ...func(error) error) (*conc.Future[struct{}], error) {
				textTask := task.(*syncmgr.TextSourceSyncTask)
				taskCh <- textTask
		...
		s.Contains(l0wb.textGrowingProgress, int64(1003))
		s.Empty(l0wb.textGrowingProgress[int64(1003)].batches)
	...
	s.Run("source_becomes_unavailable_when_building_task_retries_without_fatal", func() {
		...
		l0wb := wb.(*l0WriteBuffer)
		s.Contains(l0wb.textGrowingProgress, int64(1009))
		s.EqualValues(100, wb.GetCheckpoint().GetTimestamp())
	...
	s.Run("pending_source_retry_syncs_after_source_becomes_usable", func() {
		textSchema := s.textSchema()
		metacache := s.newTextRealMetaCache(textSchema)
		sourceState := syncmgr.TextSourcePending
		wb, err := NewL0WriteBuffer(s.channelName, metacache, s.syncMgr, &writeBufferOption{
		...
		l0wb := wb.(*l0WriteBuffer)
		s.Contains(l0wb.textGrowingProgress, int64(1007))
		s.Empty(l0wb.textGrowingProgress[int64(1007)].batches)
	})
	...
	s.Run("source_task_run_flushes_source_updates_meta_and_acks_progress", func() {
		...
		metaWriter.EXPECT().UpdateTextSourceSync(mock.Anything, mock.AnythingOfType("*syncmgr.TextSourceSyncTask")).
			Run(func(_ context.Context, task *syncmgr.TextSourceSyncTask) {
		...
		s.Contains(l0wb.textGrowingProgress, int64(1006))
		s.Empty(l0wb.textGrowingProgress[int64(1006)].batches)
	...
	s.Run("drop_close_reuses_synced_manifest_for_zero_row_text_source_task", func() {
		textSchema := s.textSchema()
		metacache := s.newTextRealMetaCache(textSchema)
		flushCalls := 0
		source := fakeTextFlushSource{
			flushFunc: func(_ context.Context, startOffset, endOffset int64, _ *syncmgr.TextFlushConfig) (*syncmgr.TextFlushResult, error) {
				flushCalls++
				s.EqualValues(0, startOffset)
				s.EqualValues(10, endOffset)
				return &syncmgr.TextFlushResult{ManifestPath: "manifest-0-10", NumRows: 10}, nil
			},
		}
		metaWriter := syncmgr.NewMockMetaWriter(s.T())
		wb, err := NewL0WriteBuffer(s.channelName, metacache, s.syncMgr, &writeBufferOption{
	...
	s.Run("source_task_error_retries_without_default_error_handler_panic", func() {
		...
		source := fakeTextFlushSource{
			flushFunc: func(context.Context, int64, int64, *syncmgr.TextFlushConfig) (*syncmgr.TextFlushResult, error) {
				flushCalls++
				if flushCalls == 1 {
					return nil, fmt.Errorf("mock text source flush error")
				}
				return &syncmgr.TextFlushResult{ManifestPath: "manifest-retry", NumRows: 10}, nil
			},
		}
		...
		wb, err := NewL0WriteBuffer(s.channelName, metacache, s.syncMgr, &writeBufferOption{
			...
			errorHandler: func(error) {
				...
				panic("text source task should not call writebuffer error handler directly")
			},
			textGrowingSourceResolver: func(segmentID int64, targetOffset int64, _ *msgpb.MsgPosition) (syncmgr.TextFlushSource, syncmgr.TextSourceState) {
				return source, syncmgr.TextSourceUsable
			},
		...
		done := make(chan *syncmgr.TextSourceSyncTask, 2)
		s.syncMgr.EXPECT().SyncData(mock.Anything, mock.AnythingOfType("*syncmgr.TextSourceSyncTask"), mock.Anything).
			RunAndReturn(func(ctx context.Context, task syncmgr.Task, callbacks ...func(error) error) (*conc.Future[struct{}], error) {
				textTask := task.(*syncmgr.TextSourceSyncTask)
				textTask.WithChunkManager(storage.NewLocalChunkManager(objectstorage.RootPath(s.T().TempDir())))
		...
		progress, ok := wb.(*l0WriteBuffer).textGrowingProgress[int64(1010)]
		s.True(ok)
		s.EqualValues(1, progress.failureCount)
		s.Contains(progress.lastFailure, "mock text source flush error")
	...
	s.Run("source_task_row_count_mismatch_retries_without_meta_update", func() {
		...
		source := fakeTextFlushSource{
			flushFunc: func(context.Context, int64, int64, *syncmgr.TextFlushConfig) (*syncmgr.TextFlushResult, error) {
				return &syncmgr.TextFlushResult{ManifestPath: "manifest-mismatch", NumRows: 9}, nil
			},
		}
		metaWriter := syncmgr.NewMockMetaWriter(s.T())
		errorHandlerCalled := false
		wb, err := NewL0WriteBuffer(s.channelName, metacache, s.syncMgr, &writeBufferOption{
			...
			errorHandler: func(error) {
				errorHandlerCalled = true
				panic("text source row count mismatch should not call writebuffer error handler directly")
			},
			textGrowingSourceResolver: func(segmentID int64, targetOffset int64, _ *msgpb.MsgPosition) (syncmgr.TextFlushSource, syncmgr.TextSourceState) {
				return source, syncmgr.TextSourceUsable
			},
		...
		s.False(errorHandlerCalled)
		progress, ok := wb.(*l0WriteBuffer).textGrowingProgress[int64(1011)]
		s.True(ok)
		s.EqualValues(1, progress.failureCount)
	...
	s.Run("source_task_uses_absolute_target_offset_after_partial_flush", func() {
		...
		source := fakeTextFlushSource{
			...
			flushFunc: func(_ context.Context, startOffset, endOffset int64, _ *syncmgr.TextFlushConfig) (*syncmgr.TextFlushResult, error) {
				...
				return &syncmgr.TextFlushResult{ManifestPath: fmt.Sprintf("manifest-%d", endOffset), NumRows: endOffset - startOffset}, nil
			},
		}
		wb, err := NewL0WriteBuffer(s.channelName, metacache, s.syncMgr, &writeBufferOption{
			idAllocator: s.allocator,
	...
	s.Run("inflight_source_sync_gets_finalized_when_segment_is_flushed", func() {
		...
		secondTaskCh := make(chan *syncmgr.TextSourceSyncTask, 1)
		callCount := 0
		s.syncMgr.EXPECT().SyncData(mock.Anything, mock.AnythingOfType("*syncmgr.TextSourceSyncTask"), mock.Anything).
			RunAndReturn(func(ctx context.Context, task syncmgr.Task, callbacks ...func(error) error) (*conc.Future[struct{}], error) {
				callCount++
		...
		s.NotContains(l0wb.textGrowingProgress, int64(1012))
	})
	...
	s.Run("sealed_pending_source_waits_without_fatal_and_retries_when_usable", func() {
		...
		s.Contains(l0wb.textGrowingProgress, int64(1005))

		sourceState = syncmgr.TextSourceUsable
		taskCh := make(chan *syncmgr.TextSourceSyncTask, 2)
		s.syncMgr.EXPECT().SyncData(mock.Anything, mock.AnythingOfType("*syncmgr.TextSourceSyncTask"), mock.Anything).
			RunAndReturn(func(ctx context.Context, task syncmgr.Task, callbacks ...func(error) error) (*conc.Future[struct{}], error) {
				textTask := task.(*syncmgr.TextSourceSyncTask)
				taskCh <- textTask
		...
		s.False(textTask.IsFlush())
		s.EqualValues(10, textTask.BatchRows())

		flushTask := <-taskCh
		s.EqualValues(1005, flushTask.SegmentID())
		s.True(flushTask.IsFlush())
		s.EqualValues(0, flushTask.BatchRows())

		s.Eventually(func() bool {
			_, ok := metacache.GetSegmentByID(1005)
			return !ok
		}, time.Second, 10*time.Millisecond)
		s.NotContains(l0wb.textGrowingProgress, int64(1005))
	})
	...
	s.Run("dropped_text_progress_builds_drop_source_task", func() {
		...
		l0wb := wb.(*l0WriteBuffer)
		s.Contains(l0wb.textGrowingProgress, int64(1004))
...
func (s *L0WriteBufferSuite) TestGetTextFlushProgress() {
	s.Run("synced_growing_source_segment_is_still_handoff_candidate_until_final_flush", func() {
		handoffProvider := &fakeTextReleaseHandoffProvider{}
		registration := syncmgr.DefaultTextSourceRegistry().Register(s.channelName, handoffProvider)
		defer syncmgr.DefaultTextSourceRegistry().Unregister(registration)
		...
		metaWriter.EXPECT().UpdateTextSourceSync(mock.Anything, mock.AnythingOfType("*syncmgr.TextSourceSyncTask")).Return(nil).Once()
		done := make(chan struct{})
		s.syncMgr.EXPECT().SyncData(mock.Anything, mock.AnythingOfType("*syncmgr.TextSourceSyncTask"), mock.Anything).
			RunAndReturn(func(ctx context.Context, task syncmgr.Task, callbacks ...func(error) error) (*conc.Future[struct{}], error) {
				textTask := task.(*syncmgr.TextSourceSyncTask)
		...
		l0wb := wb.(*l0WriteBuffer)
		s.Contains(l0wb.textGrowingProgress, int64(1200))
		s.Empty(l0wb.textGrowingProgress[int64(1200)].batches)

		progress, err := wb.GetTextFlushProgress(context.Background(), []int64{1200}, 200)
		s.NoError(err)
		s.Len(progress, 1)
		s.EqualValues(1200, progress[0].SegmentID)
		s.True(progress[0].HasGrowingProgress)
		s.EqualValues(10, progress[0].TargetOffset)
		s.Equal(metacache.FlushSourceGrowing, progress[0].SourceMode)
		s.Equal([]syncmgr.TextReleaseHandoffSegment{{SegmentID: 1200, TargetOffset: 10}}, handoffProvider.segments)
	})
	...
	s.Run("dropped_growing_source_segment_is_still_handoff_candidate_until_progress_done", func() {
		...
		l0wb := wb.(*l0WriteBuffer)
		s.Contains(l0wb.textGrowingProgress, int64(1203))
		s.Empty(l0wb.textGrowingProgress[int64(1203)].batches)

		wb.DropPartitions([]int64{0})

		progress, err := wb.GetTextFlushProgress(context.Background(), []int64{1203}, 200)
		s.NoError(err)
		s.Len(progress, 1)
		...
		s.EqualValues(10, progress[0].TargetOffset)
		s.Equal(metacache.FlushSourceGrowing, progress[0].SourceMode)
		s.Equal([]syncmgr.TextReleaseHandoffSegment{{SegmentID: 1203, TargetOffset: 10}}, handoffProvider.segments)
	})
	...
	s.Run("waits_until_fence_processed_and_returns_growing_progress", func() {
		...
		resultCh := make(chan []TextFlushSegmentProgress, 1)
		errCh := make(chan error, 1)
		ctx, cancel := context.WithTimeout(context.Background(), time.Second)
		defer cancel()
		go func() {
			progress, err := wb.GetTextFlushProgress(ctx, []int64{1201}, 300)
			if err != nil {
				errCh <- err
				return
			}
		...
		case progress := <-resultCh:
			...
			s.Equal([]syncmgr.TextReleaseHandoffSegment{{SegmentID: 1201, TargetOffset: 10}}, handoffProvider.segments)
		case err := <-errCh:
			s.NoError(err)
		case <-ctx.Done():
		...
		progress, err := wb.GetTextFlushProgress(context.Background(), nil, 300)
		s.NoError(err)
		s.Len(progress, 1)
		s.EqualValues(1201, progress[0].SegmentID)
		s.True(progress[0].HasGrowingProgress)
	...
	s.Run("writebuffer_backed_segment_has_no_growing_progress", func() {
		...
		progress, err := wb.GetTextFlushProgress(context.Background(), []int64{1202}, 200)
		s.NoError(err)
		s.Len(progress, 1)
		s.EqualValues(1202, progress[0].SegmentID)
		s.False(progress[0].HasGrowingProgress)
		s.Zero(progress[0].TargetOffset)
		s.Equal(metacache.FlushSourceWriteBuffer, progress[0].SourceMode)
		s.EqualValues(200, handoffProvider.fenceTs)
		s.Equal([]syncmgr.TextReleaseHandoffSegment{{SegmentID: 1202}}, handoffProvider.segments)
	})
}

func (s *L0WriteBufferSuite) TestTextSourceRegistryResolveMultipleProviders() {
	registry := syncmgr.NewTextSourceRegistry()
	channel := "by-dev-rootcoord-dml_1v0"

	pendingReg := registry.Register(channel, fakeTextSourceProvider{
		source: fakeTextFlushSource{currentOffset: 8},
		state:  syncmgr.TextSourcePending,
	})
	pendingReg2 := registry.Register(channel, fakeTextSourceProvider{
		source: fakeTextFlushSource{currentOffset: 9},
		state:  syncmgr.TextSourcePending,
	})
	registry.Register(channel, fakeTextSourceProvider{state: syncmgr.TextSourceUnavailable})

	resolved, state := registry.Resolve(channel, 1000, 10, &msgpb.MsgPosition{Timestamp: 200})
	s.Equal(syncmgr.TextSourcePending, state)
	s.Nil(resolved)

	usableReg := registry.Register(channel, fakeTextSourceProvider{
		source: fakeTextFlushSource{currentOffset: 10},
		state:  syncmgr.TextSourceUsable,
	})
	usableReg2 := registry.Register(channel, fakeTextSourceProvider{
		source: fakeTextFlushSource{currentOffset: 12},
		state:  syncmgr.TextSourceUsable,
	})
	resolved, state = registry.Resolve(channel, 1000, 10, &msgpb.MsgPosition{Timestamp: 200})
	s.Equal(syncmgr.TextSourceUsable, state)
	s.NotNil(resolved)
	...
	resolved, state = registry.Resolve(channel, 1000, 10, &msgpb.MsgPosition{Timestamp: 200})
	s.Equal(syncmgr.TextSourceUnavailable, state)
	s.Nil(resolved)
}

func (s *L0WriteBufferSuite) TestTextSourceRegistryResolveReleasesUnselectedSources() {
	registry := syncmgr.NewTextSourceRegistry()
	channel := "by-dev-rootcoord-dml_1v0"
	released := 0

	registry.Register(channel, fakeTextSourceProvider{
		source: fakeTextFlushSource{currentOffset: 10, releaseFunc: func() { released++ }},
		state:  syncmgr.TextSourceUsable,
	})
	registry.Register(channel, fakeTextSourceProvider{
		source: fakeTextFlushSource{currentOffset: 12, releaseFunc: func() { released++ }},
		state:  syncmgr.TextSourceUsable,
	})
	registry.Register(channel, fakeTextSourceProvider{
		source: fakeTextFlushSource{currentOffset: 11, releaseFunc: func() { released++ }},
		state:  syncmgr.TextSourcePending,
	})
