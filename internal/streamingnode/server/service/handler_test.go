...
func TestHandlerServiceGetTextFlushProgress(t *testing.T) {
	...
	wbManager.EXPECT().
		GetTextFlushProgress(mock.Anything, "vchannel", segmentIDs, uint64(200)).
		Return([]writebuffer.TextFlushSegmentProgress{
			{
				SegmentID:          1001,
				TargetOffset:       10,
	...
	resp, err := service.GetTextFlushProgress(ctx, &streamingpb.GetTextFlushProgressRequest{
	...
	assert.Equal(t, streamingpb.TextFlushSourceMode_TEXT_FLUSH_SOURCE_GROWING, resp.GetProgress()[0].GetSourceMode())
	assert.Equal(t, streamingpb.TextFlushSourceMode_TEXT_FLUSH_SOURCE_WRITE_BUFFER, resp.GetProgress()[1].GetSourceMode())
}
