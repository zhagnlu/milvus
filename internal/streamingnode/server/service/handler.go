...
// GetTextFlushProgress returns TEXT flush progress tracked by the write buffer.
func (hs *handlerServiceImpl) GetTextFlushProgress(ctx context.Context, req *streamingpb.GetTextFlushProgressRequest) (*streamingpb.GetTextFlushProgressResponse, error) {
	...
	progress, err := hs.writeBufferManager.GetTextFlushProgress(ctx, req.GetVchannel(), req.GetSegmentIds(), req.GetFenceTs())
	if err != nil {
		return nil, err
	}
	return &streamingpb.GetTextFlushProgressResponse{
		Progress: marshalTextFlushSegmentProgress(progress),
	}, nil
}

func marshalTextFlushSegmentProgress(progress []writebuffer.TextFlushSegmentProgress) []*streamingpb.TextFlushSegmentProgress {
	result := make([]*streamingpb.TextFlushSegmentProgress, 0, len(progress))
	for _, item := range progress {
		result = append(result, &streamingpb.TextFlushSegmentProgress{
			SegmentId:          item.SegmentID,
			TargetOffset:       item.TargetOffset,
			HasGrowingProgress: item.HasGrowingProgress,
			SourceMode:         marshalTextFlushSourceMode(item.SourceMode),
		})
	}
	return result
}

func marshalTextFlushSourceMode(mode metacache.FlushSourceMode) streamingpb.TextFlushSourceMode {
	switch mode {
	case metacache.FlushSourceWriteBuffer:
		return streamingpb.TextFlushSourceMode_TEXT_FLUSH_SOURCE_WRITE_BUFFER
	case metacache.FlushSourceGrowing:
		return streamingpb.TextFlushSourceMode_TEXT_FLUSH_SOURCE_GROWING
	default:
		return streamingpb.TextFlushSourceMode_TEXT_FLUSH_SOURCE_UNKNOWN
	}
}
