...
func TestSegmentAllocWorker_TextForcesV3Storage(t *testing.T) {
	paramtable.Init()
	resource.InitForTest(t)
	...
	for name, tc := range map[string]struct {
		...
		"non_text": {hasTextFields: false, expected: storage.StorageV2},
		"text":     {hasTextFields: true, expected: storage.StorageV3},
	} {
		t.Run(name, func(t *testing.T) {
			w := &segmentAllocWorker{
				...
				hasTextFields: tc.hasTextFields,
			}
			w.SetLogger(log.With())
