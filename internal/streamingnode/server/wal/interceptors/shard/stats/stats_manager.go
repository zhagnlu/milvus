...
func (m *StatsManager) registerNewGrowingSegment(belongs SegmentBelongs, stats *SegmentStats) {
	...
	m.totalFlushSize = addUint64Saturated(m.totalFlushSize, stats.FlushSize())
	m.metricHelper.ObserveFlushPressureBytesUpdate(m.totalFlushSize)
...
func (m *StatsManager) allocRows(segmentID int64, insert ModifiedMetrics, runtimeFlushSize uint64) (bool, error) {
	...
	if inserted {
		...
		m.totalFlushSize = addUint64Saturated(m.totalFlushSize, runtimeFlushSize)
		m.metricHelper.ObserveFlushPressureBytesUpdate(m.totalFlushSize)
...
func (m *StatsManager) UpdateFlushSourceMode(segmentID int64, mode metacache.FlushSourceMode) {
	...
	if mode == metacache.FlushSourceWriteBuffer {
		...
		if oldFlushSize > newFlushSize {
			m.totalFlushSize = subtractUint64Saturated(m.totalFlushSize, oldFlushSize-newFlushSize)
		} else if newFlushSize > oldFlushSize {
			m.totalFlushSize = addUint64Saturated(m.totalFlushSize, newFlushSize-oldFlushSize)
		}
		m.metricHelper.ObserveFlushPressureBytesUpdate(m.totalFlushSize)
	}
}
...
func (m *StatsManager) unregisterSealedSegment(segmentID int64) *SegmentStats {
	...
	m.totalFlushSize = subtractUint64Saturated(m.totalFlushSize, stats.FlushSize())
	m.metricHelper.ObserveFlushPressureBytesUpdate(m.totalFlushSize)
...
func (m *StatsManager) selectSegmentsUntilLessThanLWM() []int64 {
	m.mu.Lock()
	restSpace := uint64ToInt64Saturated(m.totalFlushSize) - m.cfg.growingBytesLWM
	m.mu.Unlock()
...
func addUint64Saturated(a uint64, b uint64) uint64 {
...
func subtractUint64Saturated(a uint64, b uint64) uint64 {
	if b > a {
		return 0
	}
	return a - b
