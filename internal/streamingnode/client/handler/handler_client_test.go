...
func TestHandlerClient_GetTextFlushProgress(t *testing.T) {
	assignment := &types.PChannelInfoAssigned{
		Channel: types.PChannelInfo{Name: "pchannel", Term: 1},
		Node:    types.StreamingNodeInfo{ServerID: 1, Address: "localhost"},
	}
	...
	service := mock_lazygrpc.NewMockService[streamingpb.StreamingNodeHandlerServiceClient](t)
	handlerServiceClient := mock_streamingpb.NewMockStreamingNodeHandlerServiceClient(t)
	handlerServiceClient.EXPECT().GetTextFlushProgress(mock.Anything, mock.Anything).RunAndReturn(
		func(ctx context.Context, req *streamingpb.GetTextFlushProgressRequest, co ...grpc.CallOption) (*streamingpb.GetTextFlushProgressResponse, error) {
			serverID, ok := contextutil.GetPickServerID(ctx)
			assert.True(t, ok)
			...
			return &streamingpb.GetTextFlushProgressResponse{
				Progress: []*streamingpb.TextFlushSegmentProgress{
					{
						SegmentId:          1001,
						TargetOffset:       10,
						HasGrowingProgress: true,
						SourceMode:         streamingpb.TextFlushSourceMode_TEXT_FLUSH_SOURCE_GROWING,
					},
					{
						SegmentId:    1002,
						SourceMode:   streamingpb.TextFlushSourceMode_TEXT_FLUSH_SOURCE_WRITE_BUFFER,
						TargetOffset: 0,
					},
				},
			}, nil
	...
	progress, err := handler.GetTextFlushProgress(context.Background(), vchannel, segmentIDs, 200)
	assert.NoError(t, err)
