...
TEST(Indexing, Naive) {
    ...
    auto vec_index = std::dynamic_pointer_cast<Index::VectorIndex>(index);
...
class IndexTest : public ::testing::TestWithParam<Param> {
 protected:
    ...
    init_minio() {
        ...
        config::ChunkMangerConfig::SetUseIAM(useIAM);
...
TEST_P(IndexTest, BuildAndQuery) {
    ...
    milvus::storage::DiskANNFileManagerImplPtr file_manager = nullptr;
    if (index_type == knowhere::IndexEnum::INDEX_DISKANN) {
        ...
        file_manager = std::make_shared<milvus::storage::DiskANNFileManagerImpl>(field_data_meta, index_meta);
    }
    ...
    milvus::Index::VectorIndexPtr vec_index = nullptr;
    if (index_type == knowhere::IndexEnum::INDEX_DISKANN) {
        ...
        milvus::storage::IndexMeta index_meta{3, 100, 1000, 1};
        file_manager = std::make_shared<milvus::storage::DiskANNFileManagerImpl>(field_data_meta, index_meta);
        vec_index = std::make_shared<milvus::Index::VectorDiskAnnIndex<float>>(index_type, metric_type,
                                                                               IndexMode::MODE_CPU, file_manager);
        ...
        vec_index = std::dynamic_pointer_cast<milvus::Index::VectorIndex>(index);
    }
