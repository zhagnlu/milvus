...
class MinioChunkManagerTest : public testing::Test {
 public:
    ...
    string GetConfig() {
        char testPath[100];
        return string(getcwd(testPath, sizeof(testPath))) + "/../../configs/milvus.yaml";
    }
    void SetUp() {
...
TEST_F(MinioChunkManagerTest, BucketPositive) {
    ...
    bool isExist = chunkManager_->BucketExists(testBucketName);
    EXPECT_EQ(isExist, false);
    chunkManager_->CreateBucket(testBucketName);
    isExist = chunkManager_->BucketExists(testBucketName);
...
TEST_F(MinioChunkManagerTest, ObjectExist) {
    ...
    bool isExist = chunkManager_->Exist(objPath);
    EXPECT_EQ(isExist, false);
}
...
TEST_F(MinioChunkManagerTest, WritePositive) {
    ...
    uint8_t content[5] = {0x17, 0x32, 0x45, 0x34, 0x23};
    string path = "1/3/5";
    chunkManager_->Write(path, content, sizeof(content));

    bool objExist = chunkManager_->Exist(path);
    EXPECT_EQ(objExist, true);
}
...
TEST_F(MinioChunkManagerTest, ReadPositive) {
    ...
    auto fileSize = chunkManager_->Size(path);

    uint8_t readContent[20] = {0};
    uint64_t size = chunkManager_->Read(path, readContent, 20);
    EXPECT_EQ(size, 5);
    EXPECT_EQ(readContent[0], 0x17);
    EXPECT_EQ(readContent[1], 0x32);
    EXPECT_EQ(readContent[2], 0x45);
    EXPECT_EQ(readContent[3], 0x34);
    EXPECT_EQ(readContent[4], 0x23);
}
...
TEST_F(MinioChunkManagerTest, RemovePositive) {
    ...
    objExist = chunkManager_->Exist(path);
    EXPECT_EQ(objExist, false);
}
...
TEST_F(MinioChunkManagerTest, ListWithPrefixPositive) {
    ...
    string path3 = "1/4/8";
    uint8_t content[5] = {0x17, 0x32, 0x45, 0x34, 0x23};
    chunkManager_->Write(path1, content, sizeof(content));
    chunkManager_->Write(path2, content, sizeof(content));
    chunkManager_->Write(path3, content, sizeof(content));
