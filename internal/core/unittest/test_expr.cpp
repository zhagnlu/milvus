...
TEST(Expr, TestExprs) {
    ...
    auto build_expr = [&](enum ExprType test_type) -> std::shared_ptr<query::Expr> {
        switch (test_type) {
            case UnaryRangeExpr:
                return std::make_shared<query::UnaryRangeExprImpl<float>>(float_fid, DataType::FLOAT,
                                                                          proto::plan::OpType::GreaterThan, 10);
                break;
            case TermExprImpl: {
                std::vector<int64_t> retrieve_ints = {1, 4, 6};
                return std::make_shared<query::TermExprImpl<int64_t>>(int64_fid, DataType::INT64, retrieve_ints);
                break;
            }
            case CompareExpr: {
                auto compare_expr = std::make_shared<query::CompareExpr>();
            ...
            case BinaryRangeExpr: {
                return std::make_shared<query::BinaryRangeExprImpl<int64_t>>(
                    ColumnInfo(int64_fid, DataType::INT64), proto::plan::GenericValue::ValCase::kInt64Val, true, true,
                    10, 45);
                break;
            }
            case LogicalUnaryExpr: {
                ExprPtr child_expr = std::make_unique<query::UnaryRangeExprImpl<int32_t>>(
                    int32_fid, DataType::INT32, proto::plan::OpType::GreaterThan, 10);
                return std::make_shared<query::LogicalUnaryExpr>(LogicalUnaryExpr::OpType::LogicalNot, child_expr);
                break;
            }
            case LogicalBinaryExpr: {
                ExprPtr child1_expr = std::make_unique<query::UnaryRangeExprImpl<int8_t>>(
                    int8_fid, DataType::INT8, proto::plan::OpType::GreaterThan, 10);
                ExprPtr child2_expr = std::make_unique<query::UnaryRangeExprImpl<int8_t>>(
                    int8_fid, DataType::INT8, proto::plan::OpType::NotEqual, 10);
                return std::make_shared<query::LogicalBinaryExpr>(LogicalBinaryExpr::OpType::LogicalXor, child1_expr,
                                                                  child2_expr);
                break;
            }
            case BinaryArithOpEvalRangeExpr: {
                return std::make_shared<query::BinaryArithOpEvalRangeExprImpl<int8_t>>(
                    ColumnInfo(int8_fid, DataType::INT8), proto::plan::GenericValue::ValCase::kInt64Val,
                    proto::plan::ArithOpType::Add, 10, proto::plan::OpType::Equal, 100);
                break;
            }
            default:
                return std::make_shared<query::BinaryRangeExprImpl<int64_t>>(
