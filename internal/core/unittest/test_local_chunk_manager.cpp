...
TEST_F(LocalChunkManagerTest, WriteOffset) {
    ...
    string file = "test-write";
    auto exist = chunkManager_->Exist(file);
    EXPECT_EQ(exist, false);
    chunkManager_->CreateFile(file);
    exist = chunkManager_->Exist(file);
    EXPECT_EQ(exist, true);
    ...
    chunkManager_->Write(file, offset, data, sizeof(data));
    ...
    uint8_t readdata[20] = {0};
    size = chunkManager_->Read(file, readdata, 20);
    EXPECT_EQ(size, 10);
    EXPECT_EQ(readdata[0], 0x17);
    EXPECT_EQ(readdata[1], 0x32);
    EXPECT_EQ(readdata[2], 0x00);
...
TEST_F(LocalChunkManagerTest, ReadOffset) {
    ...
    uint8_t readdata[20];
    auto size = chunkManager_->Read(file, 0, readdata, 3);
    EXPECT_EQ(size, 3);
    EXPECT_EQ(readdata[0], 0x17);
    EXPECT_EQ(readdata[1], 0x32);
    EXPECT_EQ(readdata[2], 0x00);
    size = chunkManager_->Read(file, 3, readdata, 4);
    EXPECT_EQ(size, 4);
    EXPECT_EQ(readdata[0], 0x34);
    EXPECT_EQ(readdata[1], 0x23);
    EXPECT_EQ(readdata[2], 0x23);
    EXPECT_EQ(readdata[3], 0x87);
    size = chunkManager_->Read(file, 7, readdata, 4);
    EXPECT_EQ(size, 1);
