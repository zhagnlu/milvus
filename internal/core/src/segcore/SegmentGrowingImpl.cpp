        ...
        case DataType::TEXT: {
            ...
            for (int64_t i = 0; i < count; ++i) {
                ...
                if (offset < text_loaded_row_count_) {
                    // Loaded region: raw text from scanner/WAL replay
                    // or LOBReference from V3 manifest (handled by HasTextLobPath below)
                    dst->at(i).assign(ref_str.data(), ref_str.size());
                } else if (HasTextLobSpillover(field_id)) {
            ...
            // For loaded region with V3 manifest LOBReferences,
            // resolve them via TextColumnCache in a batch
            if (HasTextLobPath(field_id) && text_loaded_row_count_ > 0) {
                ...
                for (int64_t i = 0; i < count; ++i) {
                    ...
                    if (offset < text_loaded_row_count_) {
                        ...
                        encoded_refs.push_back(
                            {reinterpret_cast<const uint8_t*>(ref_str.data()),
                             ref_str.size()});
                ...
                if (!encoded_refs.empty()) {
                    auto properties =
                        milvus::storage::LoonFFIPropertiesSingleton::
                            GetInstance()
                                .GetProperties();
                    auto fs =
        ...
        case DataType::TEXT: {
            ...
            for (int64_t i = 0; i < count; ++i) {
                ...
                if (offset == INVALID_SEG_OFFSET) {
                    ...
                    // Loaded region: raw text (will be overwritten by
                    // TextColumnCache below if HasTextLobPath)
                    auto ref_str = src.view_element(offset);
                    dst[i].assign(ref_str.data(), ref_str.size());
                } else if (HasTextLobSpillover(field_id)) {
                    // Inserted region: decode spillover LOBRef
                    auto spillover = GetTextLobSpillover(field_id);
                    auto ref_str = src.view_element(offset);
                ...
                if (!encoded_refs.empty()) {
                    ...
                    cache.ReadBatchInto(text_lob_paths_.at(field_id),
                                        ...
                                        *properties,
                                        ...
                                        &resolved);
                    for (size_t j = 0; j < loaded_indices.size(); ++j) {
                        dst[loaded_indices[j]] =
                            std::move(*resolved.Mutable(j));
                    }
                }
            }
            break;
