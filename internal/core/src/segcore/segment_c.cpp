...
void
AppendSparseRowToBM25Stats(const knowhere::sparse::SparseRow<milvus::SparseValueType>* row,
    ...
    auto element_count = byte_size / knowhere::sparse::SparseRow<
                                         milvus::SparseValueType>::element_size();
...
SerializeBM25Stats(const BM25StatsAccumulator& stats) {
    ...
    out.reserve(sizeof(version) + sizeof(stats.num_row) +
                sizeof(stats.num_token) +
...
FlushGrowingSegmentData(CSegmentInterface c_segment,
    ...
    try {
        ...
        while (current_offset < end_offset) {
            ...
            for (const auto& field_info : field_infos) {
                ...
                if (stats_iter != bm25_stats.end()) {
                    ...
                    if (!status.ok()) {
                        return milvus::FailureCStatus(
                            milvus::UnexpectedError, status.ToString());
                    }
                }
            }
        ...
        for (const auto& [field_id, stats] : bm25_stats) {
            ...
            auto full_path = fmt::format("{}/{}", writer_config.segment_path, rel_path);
            ...
            if (config->write_merged_bm25_stats) {
                ...
                auto merged_full_path =
                    fmt::format("{}/{}", writer_config.segment_path, merged_rel_path);
                write_status = WriteRawFile(fs, merged_full_path, merged_serialized);
                if (!write_status.ok()) {
                    return milvus::FailureCStatus(milvus::UnexpectedError,
                                                  write_status.ToString());
                }
