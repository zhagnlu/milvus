...
SegmentSealedImpl::mask_with_timestamps(BitsetType& bitset_chunk, Timestamp timestamp) const {
    ...
    AssertInfo(timestamps_size == get_row_count(), "Timestamp size not equal to row count");
    ...
    auto mask = TimestampIndex::GenerateBitset(timestamp, range, (Timestamp*)timestamps_data, timestamps_size);
    bitset_chunk |= mask;
}
