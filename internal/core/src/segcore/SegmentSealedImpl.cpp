...
void
print(const Index::LoadIndexInfo& info) {
    std::cout << "------------------LoadIndexInfo----------------------" << std::endl;
    std::cout << "field_id: " << info.field_id << std::endl;
    std::cout << "field_type: " << int(info.field_type) << std::endl;
...
SegmentSealedImpl::LoadVecIndex(const Index::LoadIndexInfo& info) {
    ...
    auto index = std::dynamic_pointer_cast<Index::VectorIndex>(info.index);
...
SegmentSealedImpl::LoadScalarIndex(const Index::LoadIndexInfo& info) {
    ...
    scalar_indexings_[field_id] = info.index;
    ...
    if (schema_->get_primary_field_id() == field_id) {
        ...
        switch (field_meta.get_data_type()) {
            case DataType::INT64: {
                auto int64_index = std::dynamic_pointer_cast<Index::ScalarIndex<int64_t>>(info.index);
            ...
            case DataType::VARCHAR: {
                auto string_index = std::dynamic_pointer_cast<Index::ScalarIndex<std::string>>(info.index);
                for (int i = 0; i < row_count; ++i) {
