...
void
JsonKeyStats::LoadMetaFile(const std::string& meta_file_path) {
    ...
    auto fs = milvus_storage::ArrowFileSystemSingleton::GetInstance()
                  .GetArrowFileSystem();
    auto remote_prefix =
        AddBucketName(disk_file_manager_->GetRemoteJsonStatsLogPrefix());
    std::filesystem::path full_path =
        std::filesystem::path(remote_prefix) / meta_file_path;

    auto input_stream = fs->OpenInputStream(full_path.string());
    AssertInfo(input_stream.ok(),
               "Failed to open meta file: {} for segment {}",
               full_path.string(),
               segment_id_);

    auto file_size_result = (*input_stream)->GetSize();
    AssertInfo(file_size_result.ok(),
               "Failed to get meta file size: {} for segment {}",
               full_path.string(),
               segment_id_);
    auto file_size = *file_size_result;

    std::string meta_content;
    meta_content.resize(file_size);
    auto read_result = (*input_stream)->Read(file_size, meta_content.data());
    AssertInfo(read_result.ok(),
               "Failed to read meta file: {} for segment {}",
               full_path.string(),
               segment_id_);

    json_stats_meta_ = JsonStatsMeta::Deserialize(meta_content);

    // build key_field_map from layout type map
    key_field_map_ = json_stats_meta_.BuildKeyFieldMap();
...
JsonKeyStats::Load(milvus::tracer::TraceContext ctx, const Config& config) {
    ...
    for (const auto& file : index_files.value()) {
        ...
        } else if (file.find(JSON_STATS_SHARED_INDEX_PATH) !=
                   std::string::npos) {
    ...
    if (!meta_files.empty()) {
        AssertInfo(meta_files.size() == 1,
                   "expected exactly one meta file, got {} for segment {}",
                   meta_files.size(),
                   segment_id_);
        LoadMetaFile(meta_files[0]);
    }
