...
JsonKeyStats::TraverseJsonForStats(const char* json,
                                   ...
                                   std::map<JsonKey, KeyStatsInfo>& infos) {
    //LOG_DEBUG("TraverseJsonForStats: {} for segment {}", json, segment_id_);
...
JsonKeyStats::CollectSingleJsonStatsInfo(
    ...
    //LOG_DEBUG("TraverseJsonForStats: {} for segment {}", json_str, segment_id_);
...
JsonKeyStats::ClassifyJsonKeyLayoutType(
    ...
    auto ClassifyKey = [&](const JsonKey& key,
        ...
        // for null/array/object, must be classified as shared
        if (!IsPrimitiveJsonType(key.type_) || IsNullJsonType(key.type_)) {
...
JsonKeyStats::BuildKeyStatsForRow(const char* json_str, uint32_t row_id) {
    // LOG_DEBUG("build key stats for row {} with json {} for segment {}",
    //           row_id,
    //           json_str,
    //           segment_id_);
    jsmn_parser parser;
...
JsonKeyStats::BuildWithFieldData(const std::vector<FieldDataPtr>& field_datas,
    ...
    LOG_INFO("collect key infos: {} for segment {}",
             PrintKeyInfo(infos),
             segment_id_);
    key_types_ = ClassifyJsonKeyLayoutType(infos);
    LOG_INFO("key types infos: {} for segment {}",
...
JsonKeyStats::GetColumnSchemaFromParquet(int64_t column_group_id,
    ...
    for (const auto& field : file_schema->fields()) {
        ...
        // LOG_DEBUG("metadata: {} for field: {} for segment {}",
        //           metadata->ToString(),
        //           field_name,
        //           segment_id_);

        // auto result = metadata->Get(JSON_STATS_META_KEY_LAYOUT_TYPE);
        // AssertInfo(result.ok(),
        //            "failed to get layout type from metadata for field {}: {} "
        //            "for segment {}",
        //            field_name,
        //            result.status().ToString(),
        //            segment_id_);
        // auto layout_type_str = result.ValueOrDie();
        // JsonKeyLayoutType layout_type =
        //     JsonKeyLayoutTypeFromString(layout_type_str);
        // field_key_type_map_[field_name] = layout_type;
        ...
        field_data_type_map_[field_name] = field_type;
        ...
        LOG_INFO(
            ...
            field_name,
            field_id,
            field_data_type_map_[field_name],
            segment_id_);
    }
}
...
JsonKeyStats::GetCommonMetaFromParquet(const std::string& file) {
    ...
    for (int i = 0; i < kv_metadata->size(); ++i) {
        ...
        // LOG_DEBUG("parquet metadata entry: {} = {} for segment {}",
        //          key,
        //          value,
        //          segment_id_);

        if (key == JSON_STATS_META_KEY_MAP) {
            try {
                auto key_map_json = nlohmann::json::parse(value);
                for (const auto& [k, v] : key_map_json.items()) {
                    key_field_map_[v].insert(k);
                }
            } catch (const std::exception& e) {
                ThrowInfo(ErrorCode::UnexpectedError,
                          "Failed to parse JSON_STATS_META_KEY_MAP from "
        ...
        if (key == JSON_STATS_META_KEY_LAYOUT_TYPE) {
            try {
                auto layout_type_json = nlohmann::json::parse(value);
                for (const auto& [k, v] : layout_type_json.items()) {
                    field_key_type_map_[k] = JsonKeyLayoutTypeFromString(v);
                    key_data_type_map_[k] = GetJsonTypeFromKeyName(k);
                }
            } catch (const std::exception& e) {
                ThrowInfo(
                    ErrorCode::UnexpectedError,
                    "Failed to parse JSON_STATS_META_KEY_LAYOUT_TYPE from "
                    "metadata: {} in file: {} for segment {}",
                    e.what(),
                    file,
                    segment_id_);
...
JsonKeyStats::LoadColumnGroup(int64_t column_group_id,
    ...
    for (const auto& field_id : milvus_field_ids) {
        ...
        FieldMeta field_meta(
            ...
            field_id,
            GetPrimitiveDataType(field_data_type_map_[field_name]),
            true,
            std::nullopt);
