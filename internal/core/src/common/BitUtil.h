...
template <typename T>
inline bool
isBitSet(const T* bits, int32_t idx) {
...
template <typename T>
inline void
setBit(T* bits, uint32_t idx, bool value) {
    value ? setBit(bits, idx) : clearBit(bits, idx);
}

inline void
negate(char* bits, int32_t size) {
    int32_t i = 0;
    for (; i + 64 <= size; i += 64) {
...
void
forBatches(const uint64_t* bits, int32_t begin, int32_t end, Callable func) {
...
template <typename PartialWordFunc, typename FullWordFunc>
inline bool
testWordsReverse(int32_t begin,
                 int32_t end,
                 PartialWordFunc partialWordFunc,
                 FullWordFunc fullWordFunc) {
    if (begin >= end) {
...
inline void
fillBits(uint64_t* bits, int32_t begin, int32_t end, bool value) {
    forEachWord(
        begin,
        end,
 ...
 */
inline void
reverseBits(uint8_t* bytes, int numBytes) {
    for (int i = 0; i < numBytes; ++i) {
        auto byte = bytes[i];
        bytes[i] = ((byte & 0x01) << 7) | ((byte & 0x02) << 5) |
                   ((byte & 0x4) << 3) | ((byte & 0x08) << 1) |
...
inline int32_t
findFirstBit(const uint64_t* bits, int32_t begin, int32_t end) {
    int32_t found = -1;
    testWords(
        begin,
        end,
...
void
forEachBit(const uint64_t* bits,
           int32_t begin,
           int32_t end,
           bool isSet,
           Callable func) {
...
template <typename Callable>
inline void
forEachUnsetBit(const uint64_t* bits,
                int32_t begin,
                int32_t end,
                Callable func) {
...
inline int32_t
findLastBit(const uint64_t* bits,
            int32_t begin,
            int32_t end,
            bool value = true) {
    int32_t found = -1;
...
inline void
andWithNegatedBits(uint64_t* target,
                   const uint64_t* left,
                   const uint64_t* right,
                   int32_t begin,
                   int32_t end) {
...
inline void
orWithNegatedBits(uint64_t* target,
                  const uint64_t* left,
                  const uint64_t* right,
                  int32_t begin,
                  int32_t end) {
...
bool inline hasIntersection(const uint64_t* left,
                            const uint64_t* right,
                            int32_t begin,
                            int32_t end) {
    if (begin >= end) {
...
#endif
inline uint64_t
commutativeHashMix(const uint64_t upper, const uint64_t lower) noexcept {
...
inline uint64_t
loadPartialWord(const uint8_t* data, int32_t size) {
...
inline size_t
hashBytes(size_t seed, const char* data, size_t size) {
...
template <typename T>
inline T
loadBits(const uint64_t* source, uint64_t bitOffset, uint8_t numBits) {
...
inline void
copyBits(const uint64_t* source,
    ...
    for (; i + 64 <= numBits; i += 64) {
        uint64_t word =
            detail::loadBits<uint64_t>(source, i + sourceOffset, 64);
        detail::storeBits<uint64_t>(target, targetOffset + i, word, 64);
    }
    if (i + 32 <= numBits) {
        auto lastWord =
            detail::loadBits<uint32_t>(source, sourceOffset + i, 32);
        detail::storeBits<uint32_t>(target, targetOffset + i, lastWord, 32);
        i += 32;
    }
    if (i + 16 <= numBits) {
        auto lastWord =
            detail::loadBits<uint16_t>(source, sourceOffset + i, 16);
    ...
    for (; i < numBits; i += 8) {
        ...
        detail::storeBits<uint8_t>(
            target, targetOffset + i, lastWord, copyBits);
    }
}
...
inline uint32_t
rotateLeft(uint32_t a, int shift) {
#ifdef __BMI2__
...
/// least-significant bit location, and return the unsigned result.
inline uint64_t
rotateLeft64(uint64_t a, uint32_t shift) {
