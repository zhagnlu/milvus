 ...
 * @return std::string 
 */
inline std::string ConvertFromAwsString(const Aws::String& aws_str) {
...
MinioChunkManager::MinioChunkManager(const std::string& endpoint, 
                                     ...
                                     bool secure) : default_bucket_name_(bucket_name) {
    ...
    client_ = std::make_shared<Aws::S3::S3Client>(Aws::Auth::AWSCredentials(ConvertToAwsString(access_key), 
                                                                            ConvertToAwsString(access_value)), 
                                                                            config,
                                                                            Aws::Client::AWSAuthV4Signer::PayloadSigningPolicy::Never,
                                                                            false);

    std::cout << "init MinioChunkManager with parameter[endpoint: '" << endpoint << 
                        "', access_key:'" << access_key << "', access_value:'" << access_value <<
                        "', default_bucket_name:'" << bucket_name << "', use_secure:'" << 
                        std::boolalpha << secure << "']";
}
...
uint64_t MinioChunkManager::Size(const std::string& filepath) {
	return GetObjectSize(default_bucket_name_, filepath);
}

bool MinioChunkManager::Exist(const std::string& filepath) {
	return ObjectExists(default_bucket_name_, filepath);
}

void MinioChunkManager::Remove(const std::string& filepath) {
...
uint64_t MinioChunkManager::Read(const std::string& filepath, void* buf, uint64_t size) {
...
void MinioChunkManager::Write(const std::string& filepath, void* buf, uint64_t size) {
    PutObjectBuffer(default_bucket_name_, filepath, buf, size);
}

bool MinioChunkManager::BucketExists(const std::string& bucket_name) {
...
std::vector<std::string> MinioChunkManager::ListBuckets() {
...
bool MinioChunkManager::CreateBucket(const std::string& bucket_name) {
...
bool MinioChunkManager::DeleteBucket(const std::string& bucket_name) {
...
bool MinioChunkManager::ObjectExists(const std::string& bucket_name, const std::string& object_name) {
...
int64_t MinioChunkManager::GetObjectSize(const std::string& bucket_name, const std::string& object_name) {
    Aws::S3::Model::HeadObjectRequest request;
    request.SetBucket(bucket_name.c_str());
    request.SetKey(object_name.c_str());
...
bool MinioChunkManager::DeleteObject(const std::string& bucket_name, const std::string& object_name) {
...
bool MinioChunkManager::PutObjectBuffer(const std::string& bucket_name,
