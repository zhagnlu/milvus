...
func (sd *shardDelegator) ReleaseSegments(ctx context.Context, req *querypb.ReleaseSegmentsRequest, force bool) error {
	...
	if len(growing) > 0 && sd.hasTextFields() && !force && req.GetCheckpoint().GetTimestamp() != typeutil.MaxTimestamp {
		checkpointTs := req.GetCheckpoint().GetTimestamp()
		unprepared := lo.Filter(growing, func(entry SegmentEntry, _ int) bool {
			return sd.textSourceProvider == nil || !sd.textSourceProvider.IsReleaseAllowed(entry.SegmentID, checkpointTs)
		})
		if len(unprepared) == 0 {
			log.Info("allow release TEXT growing segments after release handoff prepared",
				zap.Int64s("segmentIDs", lo.Map(growing, func(entry SegmentEntry, _ int) int64 {
					return entry.SegmentID
				})),
				zap.Uint64("checkpointTs", checkpointTs))
		} else {
			log.WithRateGroup(fmt.Sprintf("delegator.release.text.%s", sd.vchannelName), 1, 60).
				RatedWarn(30, "reject release TEXT growing segment, waiting for WAL flusher drain",
					zap.Int64("segmentCount", int64(len(unprepared))),
					zap.Int64s("unpreparedSegmentIDs", lo.Map(unprepared, func(entry SegmentEntry, _ int) int64 {
						return entry.SegmentID
					})),
					zap.Bool("needTransfer", req.GetNeedTransfer()))
			return errors.Wrap(merr.ErrServiceTooManyRequests,
				"release TEXT growing segment requires WAL fence/drain before source release; checker will retry")
		}
	} else if len(growing) > 0 && sd.hasTextFields() && !force {
		log.Info("allow release TEXT growing segments for dropped channel",
			zap.Int64s("segmentIDs", lo.Map(growing, func(entry SegmentEntry, _ int) int64 {
				return entry.SegmentID
			})))
	}
	...
	if len(growing) > 0 {
		// TEXT growing data is not flushed directly from QueryNode during
	...
	if len(growing) > 0 && sd.textSourceProvider != nil {
		for _, entry := range growing {
			sd.textSourceProvider.ClearReleasePrepared(entry.SegmentID)
		}
	}
	return nil
}
