...
func TestDelegatorGrowingSourceProviderRetainedMetrics(t *testing.T) {
	...
	segmentManager.EXPECT().ReleaseDetached(context.Background(), segment).Once()
	provider.MarkReleaseDetached(1001)
	require.Equal(t, float64(0), testutil.ToFloat64(metrics.QueryNodeGrowingSourceRetainedBytes.WithLabelValues("1", channel)))
	require.Equal(t, float64(0), testutil.ToFloat64(metrics.QueryNodeGrowingSourceRetainedSegments.WithLabelValues("1", channel)))
}
