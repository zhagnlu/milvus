...
func (node *QueryNode) UnsubDmChannel(ctx context.Context, req *querypb.UnsubDmChannelRequest) (*commonpb.Status, error) {
	...
	if ok {
		...
		preparedTextSegments := syncmgr.DefaultTextSourceRegistry().ReleasePreparedSegments(req.GetChannelName())
		...
		if detacher, ok := node.manager.Segment.(segmentDetacher); ok {
			for _, segmentID := range preparedTextSegments {
...
func (node *QueryNode) ReleaseSegments(ctx context.Context, req *querypb.ReleaseSegmentsRequest) (*commonpb.Status, error) {
	...
	for _, id := range req.GetSegmentIDs() {
		if req.GetScope() == querypb.DataScope_Streaming &&
			syncmgr.DefaultTextSourceRegistry().IsReleasePrepared(req.GetShard(), id, req.GetCheckpoint().GetTimestamp()) {
			if detacher, ok := node.manager.Segment.(segmentDetacher); ok {
				detacher.Detach(ctx, id, querypb.DataScope_Streaming)
			}
			syncmgr.DefaultTextSourceRegistry().ClearReleasePrepared(req.GetShard(), id)
			continue
		}
