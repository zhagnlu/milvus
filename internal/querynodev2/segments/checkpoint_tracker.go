...
type CheckpointTracker struct {
	...
	// key: segmentID, value: last manifest path acknowledged by DataCoord.
...
// InitSegmentWithManifest initializes tracking for a recovered segment with the last acknowledged manifest.
// The manifest version is extracted from manifestPath automatically.
func (t *CheckpointTracker) InitSegmentWithManifest(segID int64, flushedOffset int64, manifest string) {
	...
	t.flushedOffsets[segID] = flushedOffset
	if manifest != "" {
		t.acknowledgedManifests[segID] = manifest
	}
}
...
func (t *CheckpointTracker) GetAcknowledgedVersion(segID int64) int64 {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if mp, ok := t.acknowledgedManifests[segID]; ok {
		if _, version, err := packed.UnmarshalManifestPath(mp); err == nil {
			return version
		}
	}
...
// GetAcknowledgedManifest returns the last manifest path acknowledged by DataCoord.
// Returns "" if no manifest has been acknowledged yet.
...
// This represents the latest row offset that has a corresponding WAL position.
// Returns flushedOffset if no batches exist (all previous batches were cleaned up).
func (t *CheckpointTracker) GetMaxEndOffset(segID int64) int64 {
	...
	batches := t.segmentBatches[segID]
	if len(batches) > 0 {
		return batches[len(batches)-1].EndOffset
	}
	return t.flushedOffsets[segID]
}
