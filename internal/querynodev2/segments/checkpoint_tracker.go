...
type CheckpointTracker struct {
	...
	// key: segmentID, value: last manifest version acknowledged by DataCoord
	// This is updated ONLY after SaveBinlogPaths succeeds.
	// FlushData must use this version as read_version (not -1/latest) to avoid
	// reading uncommitted manifest versions and duplicating data on retry.
	acknowledgedVersions map[int64]int64
}
...
func NewCheckpointTracker() *CheckpointTracker {
	return &CheckpointTracker{
		...
		acknowledgedVersions: make(map[int64]int64),
	}
}
...
// InitSegmentWithVersion initializes tracking for a recovered segment with manifest version.
func (t *CheckpointTracker) InitSegmentWithVersion(segID int64, flushedOffset int64, manifestVersion int64) {
	t.mu.Lock()
	defer t.mu.Unlock()

	t.flushedOffsets[segID] = flushedOffset
	t.acknowledgedVersions[segID] = manifestVersion
}
...
func (t *CheckpointTracker) RemoveSegment(segID int64) {
	...
	delete(t.acknowledgedVersions, segID)
}
...
func (t *CheckpointTracker) GetAcknowledgedVersion(segID int64) int64 {
	t.mu.RLock()
	defer t.mu.RUnlock()
	if v, ok := t.acknowledgedVersions[segID]; ok {
		return v
	}
	return -1
}

// UpdateAcknowledgedVersion updates the acknowledged manifest version.
// Must be called ONLY after SaveBinlogPaths succeeds.
func (t *CheckpointTracker) UpdateAcknowledgedVersion(segID int64, version int64) {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.acknowledgedVersions[segID] = version
}
