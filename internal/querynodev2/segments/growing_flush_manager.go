...
func (m *GrowingFlushManager) doSync(ctx context.Context, segment Segment, isSeal bool) error {
	...
	if flushedOffset >= currentOffset {
		if isSeal {
			// All data already flushed but segment needs to be sealed.
			// Send SaveBinlogPaths with Flushed=true to transition segment state.
			// checkpoint may be nil here (batches cleaned up after previous flush),
			// which is fine: DmlPosition was already updated in the previous flush,
			// this request only needs to set Flushed=true.
			log.Info("no unflushed data, but sealing segment")
			return m.saveManifestAndCheckpoint(ctx, segment, "", nil, currentOffset, true)
		}
	...
	// This ensures that if a previous flush committed a manifest version but
	// SaveBinlogPaths failed, we restart from the last known-good version
	// instead of the uncommitted one, avoiding data duplication.
	...
	if err := m.saveManifestAndCheckpoint(ctx, segment, flushResult.ManifestPath, checkpoint, flushResult.NumRows, isSeal); err != nil {
		return err
	}

	// 5. update flushed offset AND acknowledged version
	// Both updates happen ONLY after SaveBinlogPaths succeeds.
	// This is the transaction boundary: before this point, a retry will
	// re-flush from the last acknowledged version (forking from the known-good state).
	m.checkpointTracker.UpdateFlushedOffset(segID, currentOffset)
	m.checkpointTracker.UpdateAcknowledgedVersion(segID, flushResult.CommittedVersion)

	log.Info("sync completed",
		zap.Int64("flushedRows", flushResult.NumRows),
		zap.String("manifestPath", flushResult.ManifestPath),
		zap.Int64("committedVersion", flushResult.CommittedVersion),
	)
...
func (m *GrowingFlushManager) saveManifestAndCheckpoint(
	...
	req := &datapb.SaveBinlogPathsRequest{
		...
		// Channel is intentionally left empty to bypass DataCoord's WAL ownership check
		// (services.go SaveBinlogPaths). QueryNode is not the WAL owner (StreamNode is),
		// so setting Channel would cause the request to be rejected.
...
func (m *GrowingFlushManager) ForceSyncAndSeal(ctx context.Context, segID int64) error {
	...
	return m.doSync(ctx, segment, true)
}
