...
func NewManifestReader(manifest string,
	...
	// // Debug: log arrow schema field types
	// for i := 0; i < arrowSchema.NumFields(); i++ {
	// 	f := arrowSchema.Field(i)
	// 	log.Info("NewManifestReader arrowSchema field",
	// 		zap.Int("idx", i), zap.String("name", f.Name),
	// 		zap.String("type", f.Type.String()), zap.Bool("nullable", f.Nullable))
	// }
