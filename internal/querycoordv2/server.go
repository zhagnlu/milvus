...
func (s *Server) initQueryCoord() error {
	...
	s.textReleaseDrainer = newTextReleaseDrainer(
		s.broker,
		s.targetMgr,
		s.startBroadcastWithCollectionIDLock,
		textFlushProgressGetterFunc(func(ctx context.Context, vchannel string, segmentIDs []int64, fenceTs uint64) ([]writebuffer.TextFlushSegmentProgress, error) {
			wal := streaming.WAL()
			if wal == nil {
				return nil, errors.New("streaming WAL is not initialized")
			}
			return wal.GetTextFlushProgress(ctx, vchannel, segmentIDs, fenceTs)
		}),
	)
