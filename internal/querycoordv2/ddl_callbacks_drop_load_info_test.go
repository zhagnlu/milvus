...
type releaseDrainProgressGetter struct {
	progress map[string][]writebuffer.TextFlushSegmentProgress
...
func (g *releaseDrainProgressGetter) GetTextFlushProgress(ctx context.Context, vchannel string, segmentIDs []int64, fenceTs uint64) ([]writebuffer.TextFlushSegmentProgress, error) {
...
func TestDrainTextRelease(t *testing.T) {
	...
	drainer := newTextReleaseDrainer(broker, targetMgr, nil, progressGetter)
...
func TestDrainTextReleaseSegmentsUsesRequestedSegmentsForHandoff(t *testing.T) {
	ctx := context.Background()
	progressGetter := &releaseDrainProgressGetter{
		progress: map[string][]writebuffer.TextFlushSegmentProgress{
			"ch1": {
	...
	drainer := newTextReleaseDrainer(nil, nil, nil, progressGetter)
	api := &releaseDrainBroadcastAPI{
		t:        t,
		channels: []string{"ch1"},
...
func TestDrainTextReleaseSegmentsSkipsEmptySegments(t *testing.T) {
	ctx := context.Background()
	progressGetter := &releaseDrainProgressGetter{}
	drainer := newTextReleaseDrainer(nil, nil, nil, progressGetter)
	api := &releaseDrainBroadcastAPI{
...
func TestDrainTextReleaseWithBroadcastResultWithoutExtra(t *testing.T) {
	ctx := context.Background()
	targetMgr := meta.NewMockTargetManager(t)
	broker := meta.NewMockBroker(t)
	progressGetter := &releaseDrainProgressGetter{
...
func TestDrainTextReleaseSkipNonText(t *testing.T) {
	drainer := newTextReleaseDrainer(nil, nil, nil, nil)
