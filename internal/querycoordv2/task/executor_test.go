...
type fakeTextReleaseDrainer struct {
	calls   []textReleaseDrainCall
	fenceTs map[string]uint64
	err     error
}

type textReleaseDrainCall struct {
	collectionID       int64
	channels           []string
...
func (d *fakeTextReleaseDrainer) DrainTextReleaseChannels(_ context.Context, collectionID int64, channels []string) (map[string]uint64, error) {
	d.calls = append(d.calls, textReleaseDrainCall{
...
func (d *fakeTextReleaseDrainer) DrainTextReleaseSegments(_ context.Context, collectionID int64, segmentsByChannel map[string][]int64) (map[string]uint64, error) {
...
func newTestExecutor(nodeID int64) *Executor {
	...
	return NewExecutor(
		...
		nil, // cluster
		nodeMgr,
		nil, // textReleaseDrainer
	)
}

func TestExecutorDrainTextBeforeStreamingSegmentRelease(t *testing.T) {
	drainErr := errors.New("drain failed")
	drainer := &fakeTextReleaseDrainer{err: drainErr}
	ex := newTestExecutor(1)
	ex.textReleaseDrainer = drainer
	...
	assert.Equal(t, []textReleaseDrainCall{{
		collectionID:       1000,
...
func TestExecutorSkipDrainTextForDroppedStreamingSegment(t *testing.T) {
	drainer := &fakeTextReleaseDrainer{err: errors.New("drain should be skipped")}
	ex := newTestExecutor(1)
	ex.textReleaseDrainer = drainer
...
func TestExecutorUsesTextReleaseFenceAsStreamingReleaseCheckpoint(t *testing.T) {
	drainer := &fakeTextReleaseDrainer{fenceTs: map[string]uint64{"ch-1": 1000}}
	ex := newTestExecutor(1)
	ex.textReleaseDrainer = drainer
	ex.targetMgr = meta.NewMockTargetManager(t)
	ex.targetMgr.(*meta.MockTargetManager).EXPECT().
...
func TestExecutorDrainTextBeforeUnsubscribeChannel(t *testing.T) {
	...
	assert.ErrorContains(t, task.Err(), "drain failed")
	assert.Equal(t, []textReleaseDrainCall{{collectionID: 1000, channels: []string{"ch-1"}}}, drainer.calls)
}
