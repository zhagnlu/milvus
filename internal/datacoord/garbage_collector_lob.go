...
func (lobCtx *lobGCContext) collectUsedLOBFiles(ctx context.Context) typeutil.Set[string] {
	...
	if snapshotMeta != nil {
		...
		for _, segment := range droppedSegments {
			...
			// conservative: if RefIndex not loaded, protect this segment's LOB files
			if !snapshotMeta.IsRefIndexLoadedForCollection(segment.GetCollectionID()) {
				lobCtx.collectLOBFilesFromSegment(ctx, segment, usedFiles)
				continue
			}
			if len(snapshotMeta.GetSnapshotBySegment(ctx, segment.GetCollectionID(), segment.GetID())) > 0 {
				lobCtx.collectLOBFilesFromSegment(ctx, segment, usedFiles)
			}
		}
	}
...
func (lobCtx *lobGCContext) collectLOBFilesFromSegment(ctx context.Context, segment *SegmentInfo, usedFiles typeutil.Set[string]) {
	if ctx.Err() != nil {
		return
	}
