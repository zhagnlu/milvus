	...
	return func(modPack *updateSegmentPack) bool {
		...
		for _, cp := range checkpoints {
			...
			if cp.GetPosition() == nil {
				// No position reported (e.g. seal-only request where data was already flushed).
				// Skip: DmlPosition and NumOfRows were already updated by previous flush.
				continue
			}
...
// segments on the given channel. This is used to prevent the channel checkpoint
// from advancing beyond unflushed growing segment data (critical for TEXT collections
// where QueryNode manages insert flush independently).
// Returns nil if no growing segments exist on the channel.
